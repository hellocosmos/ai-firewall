...
export function Policies({
  ...
  return <div className="td-policy-layout"><Panel title={t("Inspection and execution policy")} sub={t("Applied policy v{0} \xB7 changes update the local demo engine.", [policy.version])}><div className="td-form-body"><div className="td-callout"><ShieldCheck size={18} /><div><strong>{t("Saved changes apply to the next synthetic request.")}</strong><p>{integrated ? t("Applies to this installation's Envoy inspector. In-flight streams keep their original policy; new requests use the update.") : t("Does not deploy to external Envoy or customer equipment. Unmapped requests are blocked by default.")}</p></div></div><label>{t("Inspection mode")}<select value={draft.mode} onChange={e => change('mode', e.target.value)}><option value="inline">{t("Inline \u2014 allow, block, redact")}</option><option value="mirror">{t("Mirror \u2014 observation only")}</option></select></label><label>{t("Personal data handling")}<select value={draft.pii_action} onChange={e => change('pii_action', e.target.value)}><option value="redact">{t("Redact permitted fields")}</option><option value="block">{t("Block personal data")}</option></select></label><h3>{t("Tool execution rules")}</h3><div className="td-rule-list">{Object.entries(draft.rules).map(([name, value]) => <div key={name}><div><strong>{name}</strong><small>{name === 'infra.deploy' ? t("Broker approval is still required after policy allows") : name === 'notes.delete' ? t("Delete notes") : t("Read notes")}</small></div><select aria-label={t("{0} policy", [name])} value={value} onChange={e => change('rules', {
              ...draft.rules,
              [name]: e.target.value
            })}><option value="allow">{t("Allow")}</option><option value="block">{t("Block")}</option></select></div>)}</div><div className="td-form-actions"><button className="td-btn" disabled={busy} onClick={async () => {
          ...
          }}><Save size={15} />{t("Apply policy")}</button></div></div></Panel><Panel title={t("Applied scope")} sub={community?t("Community inspection"):t("Community inspection + Enterprise approval")}><div className="td-form-body"><dl className="td-dl"><dt>{t("Forwarding source")}</dt><dd>demo-decryptor</dd><dt>{t("Destination")}</dt><dd>tools.demo.test</dd><dt>{t("Path")}</dt><dd>POST /mcp</dd><dt>{t("Target field")}</dt><dd>params.arguments.message</dd><dt>{t("External URLs")}</dt><dd>{t("Empty allowlist \xB7 default deny")}</dd><dt>{t("Inspection failure")}</dt><dd>{t("Inline blocks / Mirror uncertain")}</dd></dl><p className="td-note">{integrated ? t("The destination is the installed synthetic HTTP server. Mirror observes the same path without changing content or consuming approvals. Inspector communication failures still block traffic.") : t("This form manages fixed demo mappings. Arbitrary destinations and production deployment require separate integration.")}</p></div></Panel></div>;
}
export function Connections({
  ...
  return <><div className="td-callout"><Network size={19} /><div><strong>{t("Local components and external connections are distinguished.")}</strong><p>{t("The engine and Broker are running. TLS equipment, Envoy and real IAM are not connected in this mode.")}</p></div></div><div className="td-connection-path"><span>{t("Synthetic agent")}</span><i>→</i><span>{t("Trusted-hop signature")}</span><i>→</i><span className="selected">{t("Inspection + Access Broker")}</span><i>→</i><span>{t("Decision record")}</span></div><div className="td-grid-2">{[{
        ...
        icon: KeyRound,
        title: 'Enterprise Access Broker',
        status: t("Ready"),
        kind: 'allow',
        body: t("Existing private Broker \xB7 file storage \xB7 request-bound one-time approval"),
        facts: [[t("Tenant"), 'local-demo'], [t("Registered agents"), data.broker.agents.length], [t("Delegations"), data.broker.delegations.length]]
      }, {
        icon: Server,
