...
`gateway_auth` 使用 `client_key` 或 `jwt`。JWT 模式下 TrapDefense 是 OAuth Resource Server，并发布 RFC 9728 metadata 与 `WWW-Authenticate`。`scope`/`scp` 可以是空格分隔字符串或字符串数组；可选的 `authorized_parties` 会限制 `azp`、`appid` 或 `cid`。0.39 不把 Entra `roles` app role 当作 scope。
