...
`gateway_auth`는 `client_key` 또는 `jwt`를 사용합니다. JWT 모드에서 TrapDefense는 OAuth Resource Server로 동작하며 RFC 9728 메타데이터와 `WWW-Authenticate` challenge를 제공합니다. `scope`/`scp`는 공백 구분 문자열 또는 문자열 배열을 허용하고, 선택적 `authorized_parties`는 `azp`·`appid`·`cid`를 제한합니다. Entra의 `roles` app role은 0.38에서 scope로 해석하지 않습니다.
