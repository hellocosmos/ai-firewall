...
**http://127.0.0.1:5176**에 접속합니다. 최초 계정은 **admin / 1234**입니다. **설정 → 관리자 비밀번호**에서 변경하면 모든 세션이 종료됩니다. 콘솔은 loopback에만 바인딩하는 로컬 평가용 설치본으로, 인터넷 공개용 장비가 아닙니다. 포트 충돌 시 기존 서비스를 종료하지 않고 시작을 중단합니다. Ctrl+C로 종료하면 이 설치가 소유한 Envoy 컨테이너만 제거합니다.
