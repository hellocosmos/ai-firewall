# Compatibilidad de clientes del gateway — 0.39
...
`gateway_auth` usa `client_key` o `jwt`. En modo JWT, TrapDefense es un OAuth Resource Server con metadata RFC 9728 y `WWW-Authenticate`. `scope`/`scp` puede ser una cadena separada por espacios o un array; `authorized_parties` opcional limita `azp`, `appid` o `cid`. En 0.39, los app roles de Entra en `roles` no se interpretan como scopes.
