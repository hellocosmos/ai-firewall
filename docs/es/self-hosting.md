# Autoalojamiento con Docker — 0.36 Community Preview
...
El cliente debe permitir cambiar la URL MCP/API y añadir X-TD-Client-Key. Cada despliegue tiene un único destino fijo y rutas/herramientas explícitas. Sin esas opciones, no es una integración directa.
...
| Autenticación del destino | Paso de Bearer/API Key existente o inyección de Bearer fijo desde archivo |
| No compatible | Intermediación OAuth, intercambio de tokens, cookies/sesiones, SSE prolongado, WebSocket, stdio, llamadas internas de SaaS cerrado |

La contraseña administra la consola. La clave de conexión permite entrar a TrapDefense y no prueba identidad del agente. El servicio valida su propio token y permisos. Pasar un token OAuth ya obtenido no equivale a intermediar OAuth. El SSO Entra existente de la consola fuente no está conectado a este perfil Docker.
...
Para su servicio, cambie upstream, authority, rutas, herramientas y resource en deployment.yaml. HTTPS verifica certificados; habilite HTTP expresamente solo en redes de confianza. passthrough conserva la autenticación. static_bearer requiere un archivo 0600 legible por UID 10001 y rechaza Authorization entrante. Quienes comparten la clave utilizan la misma cuenta del servicio.
