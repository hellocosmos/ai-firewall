...
TrapDefense 0.42 は単一の MIT ライセンスコードベースです。Runtime Gateway と Agent Access Broker を同じ公開リポジトリで提供し、非公開 Python 配布物、provider entry point、license key、edition switch は不要です。
...
[Docker 0.42](self-hosting.md) · [アーキテクチャ](architecture.md) · [セキュリティ](security.md) · [互換性](gateway-compatibility.md)
